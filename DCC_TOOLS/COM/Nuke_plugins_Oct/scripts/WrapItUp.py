...
# WrapItUp - 1.6
#
# Collect all media, gizmos and files associated with a nuke script, and copy it to a separate folder - along with a relinked copy of the nuke script.



WIU_Title = 'WrapItUp 1.6 - maxvanleeuwen.com'
WIU_Log = '[WrapItUp] '
...
def _Start(silent = False, nk = '', startnow = False, out = '', nodenamefolder = True, parentdircount = 3, relinked = True, relativerelinked = True, media = True, fonts = True, gizmos = True, csvcommas = False):
	...
	def PackedPath(fullPath, i = 0, nodeName = '', fontFolder = ''):
		...
		if i == 0:
			...
			newPath = (nodeName + '/' if WIU_NodeNameFolder else '')
			
			if parentDirCount > len(splitCleanPath):
			...
			# add media folder and user prefix
			newPath = WIU_PackedPath + '/' + 'MEDIA' + '/' + newPath
		...
		if i == -4:
			...
			newPath = WIU_PackedPath + '/' + 'FONTS' + '/' + fontFolder.split('/')[-1] + '/' + fullPath.split('/')[-1]
			return newPath
			...
			for eachKnob in eachNode.knobs():
				...
				if currKnob.Class() == 'File_Knob':
					...
					if foundPath is not '':
						...
						# get real paths
						realKnobPaths = GetRealKnobPaths(foundPath)
						...
						# make media item (node(s), knob(s), file exists, knob value, all paths and sizes per item, total size)
						mediaItem = [[eachNode], [eachKnob], exists, foundPath, allFilesWithSizes, totalSize]
		...
		try:
			...
			if relinkMethod is not -1:
				...
				if relinkFonts and fonts:
					vList.append(['root', 'free_type_font_path', PackedPath(WIU_OtherData[3][0], i=-4, fontFolder=WIU_OtherData[3][0])]) #font path
				...
				if media:
					for d in WIU_MediaData:
						...
						n = 0
						for eachNode in d[0]:
							if d in WIU_MediaDataNotIgnored:
								vList.append([eachNode.fullName(), d[1][n], PackedPath(d[3], nodeName=getNodeNames(d[0], us=True) )])
								n += 1
							else:
								# do not relink files that aren't copied
								pass
		...
		for j in itemsToCopy:
			...
			if k >= 0 and media:
				...
				for eachFileS in l[4]:
					...
					eachFileTo = PackedPath(eachFileS[0], nodeName = getNodeNames(l[0], us = True))

					# count sizes together
					itemSizeCounting += eachFileS[1]

					# get ratio for progress bar %
					sequenceSizeRatio = int (float(itemSizeCounting) / float(l[5]) * 100)
...
def WrapItUp(fromterminal = False, nk = '', startnow = False, out = '', nodenamefolder = True, parentdircount = 3, relinked = True, relativerelinked = True, media = True, fonts = True, gizmos = True, csvcommas = False):
	...
	if out is not '':
		...
		# start
			...
			# only print preview
			returnedFiles = _Start(silent, nk, False, out, nodenamefolder, parentdircount, relinked, relativerelinked, media, fonts, gizmos, csvcommas)
			returnedStr = ''
			...
			if startnow:
				...
				# begin
				_Start(silent, nk, startnow, out, nodenamefolder, parentdircount, relinked, relativerelinked, media, fonts, gizmos, csvcommas)
...
if __name__ == "__main__":
	...
	if t:
		...
		if err:
			...
			WrapItUp(fromterminal = True, nk = aNK, startnow = aStart, out = aOut, nodenamefolder = aNodeName, parentdircount = aDirCount, relinked = aReli, relativerelinked = aReliRela, media = aMedia, fonts = aFonts, gizmos = aGizmos, csvcommas = aCSV)
