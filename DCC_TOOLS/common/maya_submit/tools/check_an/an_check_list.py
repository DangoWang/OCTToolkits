...
def check_reference_func(fix=False):
    ...
    if illegal_references:
        if fix:
            raise_warning_dialog(None, u'以下资产未锁定！请通知资产部门进行版本锁定:\n%s' % illegal_references)
        return False
    return True


def check_locked_reference_func(fix=False):
    # 检查非法资产
    all_references = dict()
    display_refs = mc.file(q=True, r=True)
    all_ref_nodes = mc.ls(type='reference')
    ...
    if os.environ['oct_launcher_using_mode'] in ['online']:
        ...
        project_name = sg.get_project()
        for ref in display_refs:
            version_info = sg.find_shotgun("Version", [["sg_path_to_frames", "is", ref.split('{')[0]], [
        ...
        if unlocked_reference:
            if fix:
                ...
                try:
                    ...
                    print e
                for referencee in unlocked_reference:
                    for k, v in referencee.items():
                        ref = k
                        latestversion = v[0]
                        _id = v[1]
