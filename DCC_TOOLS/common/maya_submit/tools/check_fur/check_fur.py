...
import inspect
import cfx_check_list
reload(cfx_check_list)
...
def check_func(func_name, label, fix):
    result = methodcaller(func_name, fix)(cfx_check_list)
    if result:
...
class CheckCFX(base_class, form_class):
    result_signal = QtCore.Signal(int)

    def __init__(self, parent=None):
        super(CheckCFX, self).__init__(parent=getMayaWindow())
        self.setupUi(self)
        self.check_doit.clicked.connect(self.check_cfx_doit)
        self.connect_slot()
