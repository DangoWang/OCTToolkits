...
import inspect
import fl_check_list
reload(fl_check_list)
...
def check_func(func_name, label, fix=False):
    result = methodcaller(func_name, fix)(fl_check_list)
    if result:
...
form_class, base_class = loadUiType(file_path+'\\check_fl.ui')


class CheckFl(base_class, form_class):
    result_signal = QtCore.Signal(int)

    def __init__(self, parent=None):
        super(CheckFl, self).__init__(parent=getMayaWindow())
        self.setupUi(self)
        self.check_doit.clicked.connect(self.check_fl_doit)
        self.connect_slot()
