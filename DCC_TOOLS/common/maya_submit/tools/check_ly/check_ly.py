...
import inspect
import layout_check_list
reload(layout_check_list)
...
def check_func(func_name, label, fix=False):
    result = methodcaller(func_name, fix)(layout_check_list)
    if result:
...
form_class, base_class = loadUiType(file_path+'\\check_ly.ui')


class CheckLayout(base_class, form_class):
    result_signal = QtCore.Signal(int)

    def __init__(self, parent=None):
        super(CheckLayout, self).__init__(parent=getMayaWindow())
        self.setupUi(self)
        self.check_doit.clicked.connect(self.check_layout_doit)
        self.connect_slot()
    ...
    def check_layout_doit(self):
        ...
        for each in all_checking_checkboxes:
            ...
            check_func(func_name, label, False)
    ...
    def connect_slot(self):
        for each in self.get_all_checking_items():
            ...
            func_name = each.objectName().replace('_cb', '_func')
            label = self.findChild(QtWidgets.QLabel, each.objectName().replace("_cb", "_lb"))
            pb.clicked.connect(lambda func=func_name, lab=label: check_func(func, lab, True))
