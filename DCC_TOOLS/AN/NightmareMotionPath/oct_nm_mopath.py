...
form_class, base_class = loadUiType(file_path + '\\NightmareMotionPath.ui')
    ...
    def get_points_with_both_ends(self):
        ...
        for i in xrange(len(self.points_in_spans)):
            ...
            try:
                ...
                continue
            zero_weight_points = [self.crv_name+'_nm_mo_pa_jnt_'+re.findall("\d+", each)[-1]
...
class MakeCrvControls(MoPathCrv):
    ...
    def _create_jnt(self, point_num, pos):
        jnt_name = self._crv_node_name + '_nm_mo_pa_jnt_' + point_num
        return mc.joint(name=jnt_name, p=pos)

    def create_jnts(self):
        jnts_grp = mc.createNode('transform', name=self._crv_node_name + '_nm_mo_pa_jnt_grp')
    ...
    def build_controls(self):
        ...
        nm_mopath_main = self._crv_node_name + '_nm_mopath_main'
        nm_mopath_master = self._crv_node_name + '_nm_mopath_master'
        control_pos = pm.PyNode(self._crv_node_name).getRotatePivot()
        # print control_pos
        mel.eval(maya_curves.main_crv.format(**{'main_crv_name': nm_mopath_main}))
        pm.setAttr(nm_mopath_main + '.t', control_pos)
        mc.parent(controls_grp, nm_mopath_main)
        mel.eval(maya_curves.master_crv.format(**{'master_crv_name': nm_mopath_master}))
        pm.setAttr(nm_mopath_master + '.t', control_pos)
        mc.select(nm_mopath_master, r=True)
        mc.group(self._crv_node_name+'_nm_mo_pa_jnt_grp', nm_mopath_master, name=self._crv_node_name+'_ctrl_grp')
        ...
        mc.select(nm_mopath_main, nm_mopath_master, r=True)
        mel.eval('FreezeTransformations;')
        mc.parent(nm_mopath_main, nm_mopath_master)
        print 'building controls ...'


class OctNMMPath(base_class, form_class):
    def __init__(self, parent=None):
        super(OctNMMPath, self).__init__(parent=getMayaWindow())
        self.setupUi(self)
        self._ctrl_name = str()
        # self._locator_ctl = str()
        self._first_jnt_name = str()
        self._last_jnt_name = str()
        self._mo_path_name = str()
        self._crv_points_num = str()
        self._crv_roll_name = str()
        self._crv_twist_name = str()
    ...
    @QtCore.Slot(name="on_input_ctrl_twist_pb_clicked")
    def on_input_ctrl_twist_pb_clicked(self):
        selected_attr = mc.channelBox("mainChannelBox", q=1, selectedMainAttributes=1)[0]
        selected_crv = self.get_selected()
        selected_crv_attr = selected_crv + "." + selected_attr
        self.input_ctrl_twist_le.setText(selected_crv_attr)
        self._crv_twist_name = selected_crv_attr
        return selected_crv_attr
    ...
    def on_input_crv_name_pb_clicked(self):
        ...
        self._mo_path_name = selected_crv
        return selected_crv
    ...
    def on_reverse_crv_pb_clicked(self):
        if self._mo_path_name:
            mc.reverseCurve(self._mo_path_name, ch=1, rpo=1)
        return
    ...
    def rebuild_crv(self):
        ...
        if if_rebuild:
            ...
            mc.rebuildCurve(self._mo_path_name, rpo=1, ch=1, end=1, kr=0, kcp=0, kt=0, d=3, tol=0.01, rt=0, s=spans)

    def create_ik_handle(self):
        #  create locator
        control_name = self._ctrl_name.split(".")[0]
        locator_name = self._mo_path_name + "_motionPath_locator"
        ...
        #  create IK handle
        mc.select(self._first_jnt_name, self._last_jnt_name, self._mo_path_name, r=True)
        ...
        # mc.addAttr(control_name, ln='twistCon', en="off:on:", sn='tc', k=0, at='enum')
        # mc.setAttr(control_name+'.tc', cb=1)
        mc.connectAttr(self._crv_twist_name, ik_handle[0] + '.dTwistControlEnable')

        # mc.parentConstraint(self._locator_ctl, locator_grp, mo=1)
        mc.group(ik_handle[0], locator_grp, name=self._mo_path_name+'_rig_grp')
    ...
    def get_weight_mode(self):
        if self.cos_rb.isChecked():
            return 'cosx'
        elif self.x_rb.isChecked():
            return 'x'
        elif self.xx_rb.isChecked():
            return 'xx'
