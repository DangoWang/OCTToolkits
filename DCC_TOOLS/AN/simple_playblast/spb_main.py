...
class DsfSimplePlayBlast(ui.PlayBlastDialog):
    ...
    def on_playblast_doit_clicked(self, artist=None, mov_name=None, mov_path=None):
        ...
        frame = start_frame
        sound_path = "no_sound"
        ffmpeg_path_local = mc.internalVar(usd=True) + "ffmpeg.exe"
        if not os.path.isfile(ffmpeg_path_local):
            if not os.path.isdir(ffmpeg_path_local.strip('/ffmpeg.exe')):
                try:
                    os.mkdir(ffmpeg_path_local.strip('/ffmpeg.exe'))
                except:
                    # ffmpeg_path = ffmpeg_path
                    pass
                    # logging.error(u"请确保我的文档中有maya文件夹！")
                    # return (False, u"请确保我的文档中有maya文件夹！")
            try:
                shutil.copyfile(ffmpeg_path, ffmpeg_path_local)
        ...
        # 显示材质
        try:
            mc.modelEditor(current_panel, edit=1, da="smoothShaded", displayTextures=1)
        except:
            pass
    ...
    def capture(panel=None, width=None, height=None, percent=100, filename=None, frame=None, form='image',
        ...
        print dict(panel=panel, width=width, height=height, percent=100, filename=filename, frame=frame, form='image',
                compression='png', quality=100, clear_cache=1, off_screen=off_screen, viewer=viewer, show_ornaments=show_ornaments,
                overwrite=True, framePadding=4)
